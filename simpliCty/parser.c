...
TokenInfo* readSymbolTable(const char* filename, size_t* token_count) {
    ...
    *token_count = 0;
    ...
    while (fgets(line, sizeof(line), file)) {
        if (strstr(line, "TOKEN:") && strstr(line, "TYPE:") && strstr(line, "LINE:")) {
            ...
            char* linePart = strtok(NULL, "|");

            // Process the "TYPE" value
            if (typePart) {
                char type[100];
                if (sscanf(typePart, " TYPE: %s", type) == 1) {
                    if (strcmp(type, "COMMENT") == 0) { // Ignore COMMENT tokens
                        continue;
                    }
            ...
            if (linePart) {
                char line_num[100];
                if (sscanf(linePart, " LINE: %s", line_num) == 1) {
                    tokens[*token_count].line_num = strtol(strdup(line_num), NULL, 10);
                }
            }

            // printf("%s %s %d", tokens[*token_count].value, tokens[*token_count].type, tokens[*token_count].line_num);
            (*token_count)++;
        }
    }
...
// Visualized leftmost derivation parsing in parsed.txt
void writeParsingState() {
    // if no file found
...
// Match the current token with the expected type and advance token
int match(const char *expectedType, int isOptional) {
...
TreeNode* parseSimplicity() {
    ...
    if (!typeSpec) {
        fprintf(stderr, "LINE %d :: Error: Expected TYPE_SPEC\n", tokens[currentTokenIndex].line_num);
        freeTree(root);
        return NULL;
    }
    ...
    // KW_MAIN
    if (!match("KW_MAIN",0)) {
        fprintf(stderr, "LINE %d :: Error: Expected 'main'\n", tokens[currentTokenIndex].line_num);
        freeTree(root);
        return NULL;
    }
    ...
    if (!block) {
        fprintf(stderr, "LINE %d :: Error: Expected block\n", tokens[currentTokenIndex].line_num);
        freeTree(root);
        return NULL;
    }
    addChild(root, block);
...
TreeNode* parseDeclStmt() {
    ...
    // For handling backtracking (can be used for error recovery)
    size_t savedIndex = currentTokenIndex;
...
TreeNode* parseVarDecl() {
    ...
    if (idList) {
        ...
        fprintf(stderr, "LINE %d :: Error: Expected ID_LIST or IDENTIFIER\n", tokens[currentTokenIndex].line_num);
        freeTree(varDecl);
        return NULL;
    }
    ...
    if (!match("SEMICOLON", 0)) {
        fprintf(stderr, "LINE %d :: Error: Expected SEMICOLON\n", tokens[currentTokenIndex].line_num);
        freeTree(varDecl);
        return NULL;
    }
...
TreeNode* parseBoolExp() {
    ...
    // Check for repeated LOG_OR BOOL_TERM 
...
TreeNode* parseBoolTerm() {
    ...
    // Check for repeated LOG_AND BOOL_FACTOR
...
void runParser(const char* symbol_table_file) {
    ...
    FILE* logFile = freopen("output/error_log.txt", "w", stderr);
    if (!logFile) {
    ...
    // Open parsed.txt for writing leftmost derivation tracing
    ...
    if (parseTree) {
        printf("=============================================================\n"); 
        printf("ACCEPTED: Parsing successful!\n");
        printf("=============================================================\n"); 
        ...
        if (txtFile) {
            writeParseTree(txtFile, parseTree, 0);
            fclose(txtFile);
        } else {
        ...
        // Log at terminal
        printf("=============================================================\n"); 
        printf("LINE %d :: ERROR: Parsing failed at token %zu: %s\n", 
               tokens[currentTokenIndex].line_num,
               currentTokenIndex, 
        ...
        // Log at error_log file
        fprintf(stderr, "LINE %d :: ERROR: Parsing failed at token %zu: %s\n", 
               tokens[currentTokenIndex].line_num,
               currentTokenIndex, 
               currentTokenIndex < token_count ? tokens[currentTokenIndex].type : "END");
        
        // Log at parsed.txt file
...
void writeParseTree(FILE* file, TreeNode* node, int depth) {
    ...
    if (node->childCount > 0) {
        ...
        for (size_t i = 0; i < node->childCount; i++) {
            writeParseTree(file, node->children[i], depth + 1);
        }

        // close parenthessi for child nodes 
        for (int i = 0; i < depth; i++) {
            fprintf(file, "  ");
        }
        fprintf(file, ")\n");

    } else { // closing leaf nodes
        fprintf(file, ")\n");
    }
}